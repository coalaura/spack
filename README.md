...
Packing 81.8 million strings (which originally occupy 3.05 GB of heap space for slice/string headers and characters) takes about 40 seconds.

The process outputs a flat 1.14 GB byte blob, which is a 62.63% reduction in raw string data. Including the compact 5-byte pointers, the total in-memory size is 1.55 GB, yielding an overall 49.20% memory footprint reduction. The compression/packing stage runs with a net peak heap allocation of about 3.7 GB over the dataset baseline.
